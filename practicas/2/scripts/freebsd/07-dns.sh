...
acl internal-network { 192.168.25.0/24; };
options {
        forwarders {
                193.145.233.6;
                193.145.233.5;
...
// the proper IP address, or delete this option.
        listen-on       { 127.0.0.1; 192.168.25.11; };
...
# 1. Zona directa cambiada a "bsd.asorc.org"
# 2. Zona inversa
cat << 'EOF' > /usr/local/etc/namedb/internal-zones.conf
zone "bsd.asorc.org" {
	type primary;
	file "/usr/local/etc/namedb/primary/db.bsd.asorc.org";
	allow-update { none; };
};
zone "25.168.192.in-addr.arpa" {
	type master;
	file "/usr/local/etc/namedb/primary/db.192.168.25";
	allow-update { none; };
};
EOF

# Zona directa
cat << 'EOF' > /usr/local/etc/namedb/primary/db.bsd.asorc.org
$TTL 604800
@   IN  SOA     ns.bsd.asorc.org. root.bsd.asorc.org. (
2         ; Serial
604800         ; Refresh
86400         ; Retry
2419200         ; Expire
...
@   IN  NS      ns.bsd.asorc.org.

; Registros A (Direcciones IP)
@       IN  A   192.168.25.11
ns      IN  A   192.168.25.11
host    IN  A   192.168.25.1
truenas IN  A   192.168.25.9
EOF

# Zona inversa
cat << 'EOF' > /usr/local/etc/namedb/primary/db.192.168.25
$TTL 604800
@   IN  SOA     ns.bsd.asorc.org. root.bsd.asorc.org. (
1         ; Serial
604800         ; Refresh
...
11  IN  PTR     ns.bsd.asorc.org.
1   IN  PTR     host.bsd.asorc.org.
9   IN  PTR     truenas.bsd.asorc.org.
EOF

# Validación de sintaxis
named-checkconf
named-checkzone bsd.asorc.org /usr/local/etc/namedb/primary/db.bsd.asorc.org
named-checkzone 25.168.192.in-addr.arpa /usr/local/etc/namedb/primary/db.192.168.25
