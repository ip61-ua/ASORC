...
tar -czvpf $P3ASORC_BACKUP_D_DST/$P3ASORC_BACKUP_PREFIX$(date +%F).tar.gz $P3ASORC_BACKUP_D_SRC -N "$(date -d 'yesterday' +%F)" >> $P3ASORC_BACKUP_LOGFILE 2>&1
...
echo "0 3 * * 0 tar -czpf $P3ASORC_BACKUP_A_DST/FULL_\$(date +\%F).tar.gz $P3ASORC_BACKUP_A_SRC" >> mi_cron_temp
echo "0 3 * * 0 rsync -av --delete $P3ASORC_BACKUP_S_SRC/ $P3ASORC_BACKUP_S_DST/" >> mi_cron_temp
crontab $P3ASORC_BACKUP_CRONFILE

# PASO 6 (OPCIONAL): Quitar cron
rm mi_cron_temp
